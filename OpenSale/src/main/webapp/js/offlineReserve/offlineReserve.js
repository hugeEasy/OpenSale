...
//	$(".btn-search").click(function(){
	...
	$('#projectSelect').click(function(){
		...
		if(type==2){//会议室
			...
			if($('#datetimepicker5').val().trim()==""||$('#datetimepicker6').val().trim()==""||$('#datetimepicker4').val().trim()==""){
		...
		$.ajax({
			...
			success : function(data) {
				...
				for(var i=0;i<data.length;i++){
					...
					htm +="<tr projectId='"+data[i].projectId+"' pjId="+projectId+" projectName="+projectName+" type="+type+" id='productId"+data[i].projectId+"'><td>"+projectName+"</td><td productType='"+data[i].productType+"' productArea='"+data[i].productArea+"'>"+data[i].productName+"</td><td>"+spaceType+"</td><td>"+data[i].capacity+"</td><td dataUnit='"+data[i].unitEnum+"' dataUnitEnum='"+data[i].unit+"' dataMoney='"+data[i].money+"'>"+priceSign+data[i].money+signString+data[i].unitEnum+"</td><td dataCount='"+data[i].count+"'>"+data[i].count+"</td>" +
					...
					for(var j=0;j<data[i].incrementProductOutsList.length;j++){
						htm+="<input type='hidden' class='inputVal'  productId='"+data[i].incrementProductOutsList[j].productId+"' incrementLevel='"+data[i].incrementProductOutsList[j].incrementLevel+"' ratePriceFee='"+data[i].incrementProductOutsList[j].ratePriceFee+"' unit='"+data[i].incrementProductOutsList[j].unit+"' discountPrice='"+data[i].incrementProductOutsList[j].discountPrice+"' discountProductType='"+data[i].incrementProductOutsList[j].discountProductType+"' discountProductId='"+data[i].incrementProductOutsList[j].discountProductId+"' incrementProductName='"+data[i].incrementProductOutsList[j].incrementProductName+"' >";
					}
					htm+="<div id='seat-alertId' class='seat-alert'><div class='seat-header'>选择座位</div>";
					
					htm += "<div class='seat-con'><ul>" ;
					for(var j=0;j<data[i].productAssettInfoList.length;j++){
						htm += "<li class='clearfix'><div class='seat-con-l'><p class='h'>"+data[i].productAssettInfoList[j].floor+"楼:</p></div>";
						htm+="<div class='seat-con-r'>";
						for(var x=0;x<data[i].productAssettInfoList[j].assetNumber.length;x++){
							htm+="<span class='col'><a href='javascript:;' assetId='"+data[i].productAssettInfoList[j].seattId[x]+"'  class='b spana'>"+data[i].productAssettInfoList[j].assetNumber[x]+"</a></span>";
						}
						htm+="</div></li>";
					}
...
function getDiscountPrice(price,code,productType,productId){//解析优惠码  返回产品折后价价格
	var discountPrice = "0";
	if(code=="00"){
		return price;
	}else{
	...
	$.ajax({
		...
		success : function(data) {
			...
			for(var i=0;i<data.length;i++){
				...
				htm +="<tr class='person-list' userId='"+data[i].userId+"'  companyId='' userMail='"+data[i].userMail+"'  ><td><i class='glyphicon glyphicon-user'></i></td><td class='td-name'>"+username+"</td>"
				+ "<td class='td-number'>"+ data[i].userPhone+"</td><td><div class='dropdown choose-name'><button class='btn btn-default dropdown-toggle filling-name' type='button' id='dropdownMenu1' data-toggle='dropdown' aria-haspopup='true' aria-expanded='true'>"
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(!data){
				...
				/*$('.selectStagesStr').remove();
				$('#oederPageInfo').append("<p class='selectStagesStr'>尊享付款节奏折扣：96折</p>");
				getByStagesData(2);*/
				$('#stagesData').find('p').text("");
			}else{
				/*$('.selectStagesStr').remove();
				if(selectedVal==5)$('#oederPageInfo').append("<p class='selectStagesStr'>尊享付款节奏折扣：96折</p>");
				if(selectedVal==3)$('#oederPageInfo').append("<p class='selectStagesStr'>尊享付款节奏折扣：97折</p>");
				if(selectedVal==2)$('#oederPageInfo').append("<p class='selectStagesStr'>尊享付款节奏折扣：98折</p>");*/
				getByStagesData(2);
			}
	...
	$.ajax({
		...
		success : function(data) {
			if(mark==1){
				...
				var totalStr = "";
				if(resultTotal!=0) totalStr = "总金额:￥"+resultTotal+"  ";
				if(meetingBag!=0) totalStr = totalStr + meetingBag +"个会议室券   ";
				if(peopleDayBag!=0) totalStr = totalStr + peopleDayBag +"张弹性工位券";
...
function userSelected(t){
	...
	if(text.trim()=='选择公司'||text.length==0){
		...
		//$('.order-left').find('button').attr('disabled',true);
	}
	allocationUser()
}
...
function packageOptionClick(currentThis){
	...
	var packageType = 5;
//	packagePrice = $(currentThis).find("option:selected").val()=="点击选择"?$(currentThis).find("option:selected").val():getDiscountPrice($(currentThis).find("option:selected").val(),favourCode,5,packageId);  //智慧包原价   优惠码折扣加在这里
	...
	addPackageInfo(packageNoId, packageType, packageName,packagePrice,packageId,packageUnit,packageDiscountPrice,discountProductType,discountProductId,incrementLevel);
	//获取电信设置费
...
//添加智慧宝信息
function addPackageInfo(packageNoId,packageType,packageName,packagePrice,packageId,packageUnit,packageDiscountPrice,discountProductType,discountProductId,incrementLevel){
	for(var i=0;i<stationArr.length;i++){
...
function showAlert(t){
	...
	$(".sure-seat").click(function(){
		...
		for(var i=0;i<input.length;i++){
			...
			tml+="<option packageIdData='"+input.eq(i).attr('productId')+"' value='"+input.eq(i).attr('ratePriceFee')+"' packageUnit='"+input.eq(i).attr('unit')+"' incrementLevel='"+input.eq(i).attr('incrementLevel')+"'  packageDiscountPrice='"+input.eq(i).attr('discountPrice')+"' discountProductType='"+input.eq(i).attr('discountProductType')+"' discountProductId='"+input.eq(i).attr('discountProductId')+"' >"+input.eq(i).attr('incrementProductName')+" "+input.eq(i).attr('ratePriceFee')+"/"+input.eq(i).attr('unit')+"</option>";
		}
		...
		var timeEnough = "no";
		/*if(getDays(startTime,endTime)+1>=(89)){
			timeEnough = "yes";
		}*/
		...
		if(selectNum>0&&idAndSelectedArr!=null){
			pushStationArrData(new ObjStory(indexObjStory++,markId, projectName,projectId,productName,typeTitle, type,idAndSelectedArr[1],idAndSelectedArr[0], price,volume, null,null,null,null,null,null,timeEnough,startTime,endTime,unit,unitEnum,null,existPackage,0,0,0,0,null,idAndSelectedArr[2],productType,productArea,0,0,null,null,0,false,0,0))
			console.log(stationArr)
			calculateMethod();
		}
	 ...
	 for (var i = 0; i < stationArr.length; i++) {
		if (stationArr[i].packagePrice != null) {
			if(stationArr[i].upPackageObj!=null){//计算升级之后的智慧包的价格
				...
				var favourPriceFee = getDiscountPrice(stationArr[i].upPackageObj.packagePrice, favourCode, stationArr[i].upPackageObj.packageType, stationArr[i].upPackageObj.packageId);
				//alert(favourPriceFee)
				var packagePrice = favourPriceFee * stationArr[i].seatArr.length*stationArr[i].volume; 
				var calculatePackageResult = calculateDatePrice(stationArr[i].startTime, stationArr[i].endTime, packagePrice, stationArr[i].upPackageObj.packageUnit,stationArr[i].upPackageObj.packageType);
				...
				var packagePrice = favourPackagePrice
				* stationArr[i].seatArr.length*stationArr[i].volume;
				var calculatePackageResult = calculateDatePrice(stationArr[i].startTime, stationArr[i].endTime, packagePrice, stationArr[i].packageUnit,stationArr[i].packageType);
				packagePriceTotal = addFn(packagePriceTotal,Math.floor(calculatePackageResult));
				stationArr[i].calculatePackageResult = Math.floor(calculatePackageResult);
			}
		}
	 ...
	 for (var i = 0; i < stationArr.length; i++) {
		...
		if (stationArr[i].projectName == projectName&&stationArr[i].seatArr.length>0) {
			...
			if(stationArr[i].type==2){//计算会议室券
				...
				//price = stationArr[i].price * stationArr[i].seatArr.length * stationArr[i].volume;
				...
				//alert(favourPrice)
				//price = stationArr[i].price * stationArr[i].seatArr.length ;  //价格  乘以优惠券折扣 加在这里
				price = favourPrice * stationArr[i].seatArr.length ;  //价格  乘以优惠券折扣 加在这里
				if(stationArr[i].packagePrice!=null){
					...
					if(stationArr[i].discountProductType==6){//具有减免房租的特性
						...
						if(stationArr[i].upPackageDiscountObj!=null){
							...
							var favourDiscountPrice = getDiscountPrice(stationArr[i].upPackageDiscountObj.upPackageDiscountPrice, favourCode, stationArr[i].upPackageDiscountObj.upPackageType, stationArr[i].upPackageDiscountObj.upPackageId);
							//alert(favourDiscountPrice)
							discountPrice =  favourDiscountPrice * stationArr[i].seatArr.length * stationArr[i].volume;
							tempValPackage = calculateDatePrice(stationArr[i].startTime,stationArr[i].endTime,discountPrice,stationArr[i].upPackageDiscountObj.upPackageUnit,stationArr[i].type);
							if(stationArr[i].favorableLastMonth) tempValPackage = tempValPackage - discountPrice;
							//alert(stationArr[i].upPackageObj.packageDiscountPrice)
							var favourDiscountPrice = getDiscountPrice(stationArr[i].upPackageObj.packageDiscountPrice, favourCode, stationArr[i].upPackageObj.upPackageType, stationArr[i].upPackageObj.upPackageId);
							//alert(favourDiscountPrice)
							discountPrice =  favourDiscountPrice * stationArr[i].seatArr.length * stationArr[i].volume;
							valPackage = calculateDatePrice(stationArr[i].startTime,stationArr[i].endTime,discountPrice,stationArr[i].upPackageObj.packageUnit,stationArr[i].type);
							if(stationArr[i].favorableLastMonth) valPackage = valPackage - discountPrice;
							valPackage = valPackage+tempValPackage;
						}else{
							//alert(stationArr[i].packageDiscountPrice)
							var favourPackageDiscountPrice = getDiscountPrice(stationArr[i].packageDiscountPrice, favourCode, stationArr[i].packageType, stationArr[i].packageId)
							//alert(favourPackageDiscountPrice)
							discountPrice =  favourPackageDiscountPrice * stationArr[i].seatArr.length * stationArr[i].volume;
							valPackage = calculateDatePrice(stationArr[i].startTime,stationArr[i].endTime,discountPrice,stationArr[i].unit,stationArr[i].type);
							if(stationArr[i].favorableLastMonth) valPackage = valPackage - discountPrice;
						}
...
function calculateDisposableCost(projectId,productId,spaceId,productType){
	...
	$.ajax({
		...
		success : function(data) {
			for(var i=0;i<data.length;i++){
				if(data[i].productType==productType){//办公室一次性设置费用 productType = 10
					obj = data[i]; 
					var money = 0;
					if(data[i].unit ==1)
					...
					var newO = new DisposableProductObj(projectId,data[i].productId,data[i].productName,data[i].validDate,data[i].expireDate,data[i].capacity,data[i].rateId,data[i].ratePriceFee,data[i].unit,data[i].productType,productId+spaceId)
...
function OrderCommitParams(splitSign,projectId,productId,productType,areaDiscount,productDiscount,companyName,favourCode,favourDiscount,money,startTime,endTime,totalMoney,parentId,assetsIds,timeEnough,orderNo,orderTime,userId,companyId,unit,volume,margin,packageFavor,spaceType,offsetPrice,parentNo,isPackageUp,favorableLastMonth){
...
function getOrderCommitParamsArr(){
	...
	for(var i=0;i<stationArr.length;i++){
		...
		//var deposit = stationArr[i].price * stationArr[i].seatArr.length * stationArr[i].volume;//产品的价格
		console.log(stationArr[i].id)
		var favourPrice = getDiscountPrice(stationArr[i].price, favourCode, stationArr[i].productType, stationArr[i].id);////
		var deposit = favourPrice * stationArr[i].seatArr.length;//产品的价格
		var parentNo = index;
		if(stationArr[i].packageName!=null){
			arr.push(new OrderCommitParams(splitSign, stationArr[i].projectId, stationArr[i].id,stationArr[i].productType, areaDiscountMarkTemp, productDiscountTemp,  allotUserArr[3], favourCode, rebatePercent, deposit, stationArr[i].startTime, stationArr[i].endTime,0, 0, seatArrIdStr,stationArr[i].timeEnough,orderNo,orderTime,allotUserArr[0],allotUserArr[1],stationArr[i].unitEnum,stationArr[i].volume,stationArr[i].margin,stationArr[i].packageFavor,stationArr[i].type,stationArr[i].price,index++,null,stationArr[i].favorableLastMonth));
			if(stationArr[i].type==3){//disposableProduct
				for(var j=0;j<disposableProduct.length;j++){
					...
					if(disposableProduct[j].projectId==stationArr[i].projectId&&disposableProduct[j].productType==10){
						var favourPriceFee = getDiscountPrice(disposableProduct[j].ratePriceFee, favourCode, disposableProduct[j].productType, disposableProduct[j].productId);
						var disposableMoney = Math.ceil(stationArr[i].productArea) * favourPriceFee * stationArr[i].seatArr.length;;
						arr.push(new OrderCommitParams(splitSign, stationArr[i].projectId,disposableProduct[j].productId, disposableProduct[j].productType, areaDiscountMarkTemp, productDiscountTemp,  allotUserArr[3], favourCode, rebatePercent, disposableMoney, stationArr[i].startTime, stationArr[i].endTime, 0, parentNo,seatArrIdStr,stationArr[i].timeEnough,orderNo,orderTime,allotUserArr[0],allotUserArr[1],disposableProduct[j].unit,disposableProduct[j].capacity,0,0,stationArr[i].type,disposableProduct[j].ratePriceFee,index++,null,stationArr[i].favorableLastMonth));
					}
				}
			}
			...
			if(stationArr[i].upPackageObj!=null){//升级之后的智慧包
				var favourPackagePrice = getDiscountPrice(stationArr[i].upPackageObj.packagePrice, favourCode, stationArr[i].upPackageObj.packageType, stationArr[i].upPackageObj.packageId)
				var procePackagePrice = favourPackagePrice * stationArr[i].volume * stationArr[i].seatArr.length;//智慧包价格
				arr.push(new OrderCommitParams(splitSign, stationArr[i].projectId,stationArr[i].upPackageObj.packageId, stationArr[i].upPackageObj.packageType, areaDiscountMarkTemp, productDiscountTemp,  allotUserArr[3], favourCode, rebatePercent, procePackagePrice, stationArr[i].startTime, stationArr[i].endTime, 0, parentNo,seatArrIdStr,stationArr[i].timeEnough,orderNo,orderTime,allotUserArr[0],allotUserArr[1],stationArr[i].upPackageObj.packageUnit,stationArr[i].volume,0,0,stationArr[i].upPackageObj.packageType,stationArr[i].upPackageObj.packagePrice,index++,null,stationArr[i].favorableLastMonth));
			}else{
				var favourPackagePrice = getDiscountPrice(stationArr[i].packagePrice, favourCode, stationArr[i].packageType, stationArr[i].packageId);
				var procePackagePrice = favourPackagePrice * stationArr[i].volume * stationArr[i].seatArr.length;//智慧包价格
				arr.push(new OrderCommitParams(splitSign, stationArr[i].projectId,stationArr[i].packageId, stationArr[i].packageType, areaDiscountMarkTemp, productDiscountTemp,  allotUserArr[3], favourCode, rebatePercent, procePackagePrice, stationArr[i].startTime, stationArr[i].endTime, 0, parentNo,seatArrIdStr,stationArr[i].timeEnough,orderNo,orderTime,allotUserArr[0],allotUserArr[1],stationArr[i].packageUnit,stationArr[i].volume,0,0,stationArr[i].packageType,stationArr[i].packagePrice,index++,null,stationArr[i].favorableLastMonth));
			}
			if(stationArr[i].discountProductType==6){
				var packageDiscountPrice = getDiscountPrice(stationArr[i].packageDiscountPrice, favourCode, stationArr[i].discountProductType, stationArr[i].discountProductId)
				...
				if(stationArr[i].upPackageDiscountObj!=null){
					var packageDiscountPrice = getDiscountPrice(stationArr[i].upPackageObj.packageDiscountPrice, favourCode, stationArr[i].upPackageObj.discountProductType, stationArr[i].upPackageObj.discountProductId);
					discountPrice = packageDiscountPrice * stationArr[i].volume * stationArr[i].seatArr.length;
					arr.push(new OrderCommitParams(splitSign, stationArr[i].projectId,stationArr[i].upPackageObj.discountProductId, stationArr[i].upPackageObj.discountProductType, areaDiscountMarkTemp, productDiscountTemp,  allotUserArr[3], favourCode, rebatePercent, discountPrice, stationArr[i].startTime, stationArr[i].endTime, 0, parentNo,seatArrIdStr,stationArr[i].timeEnough,orderNo,orderTime,allotUserArr[0],allotUserArr[1],stationArr[i].upPackageObj.packageUnit,stationArr[i].volume,0,0,stationArr[i].upPackageObj.packageType,stationArr[i].upPackageObj.packageDiscountPrice,index++,"no",stationArr[i].favorableLastMonth));
				}else{
					arr.push(new OrderCommitParams(splitSign, stationArr[i].projectId,stationArr[i].discountProductId, stationArr[i].discountProductType, areaDiscountMarkTemp, productDiscountTemp,  allotUserArr[3], favourCode, rebatePercent, discountPrice, stationArr[i].startTime, stationArr[i].endTime, 0, parentNo,seatArrIdStr,stationArr[i].timeEnough,orderNo,orderTime,allotUserArr[0],allotUserArr[1],stationArr[i].packageUnit,stationArr[i].volume,0,0,stationArr[i].packageType,stationArr[i].packageDiscountPrice,index++,"no",stationArr[i].favorableLastMonth));
				}
			}
			//设置智慧包的升级产品
			if(stationArr[i].upPackageDiscountObj!=null){//存在智慧包的升级产品
				...
				var upPackageDiscountPrice = getDiscountPrice(upPackageDiscountObj.upPackageDiscountPrice, favourCode, upPackageDiscountObj.upPackageType, upPackageDiscountObj.upPackageId);
				var discountPrice = upPackageDiscountPrice * stationArr[i].volume * stationArr[i].seatArr.length;//智慧包价格
				arr.push(new OrderCommitParams(splitSign, stationArr[i].projectId,upPackageDiscountObj.upPackageId, upPackageDiscountObj.upPackageType, areaDiscountMarkTemp, productDiscountTemp,  allotUserArr[3], favourCode, rebatePercent, discountPrice, stationArr[i].startTime, stationArr[i].endTime, 0, parentNo,seatArrIdStr,stationArr[i].timeEnough,orderNo,orderTime,allotUserArr[0],allotUserArr[1],upPackageDiscountObj.upPackageUnit,stationArr[i].volume,0,0,stationArr[i].packageType,upPackageDiscountObj.upPackageDiscountPrice,index++,"",stationArr[i].favorableLastMonth));
			}
			//设置一次性电信设置费
			for(var j=0;j<disposableProduct.length;j++){
				//disposableProductObj(productId,productName,validDate,expireDate,capacity,rateId,ratePriceFee,unit,productType){
				if(disposableProduct[j].projectId==stationArr[i].projectId&&disposableProduct[j].productType==11){
					var ratePriceFee = getDiscountPrice(disposableProduct[j].ratePriceFee, favourCode, disposableProduct[j].productType, disposableProduct[j].productId);
					var disposableMoney =  disposableProduct[j].ratePriceFee *  stationArr[i].volume * stationArr[i].seatArr.length;
					arr.push(new OrderCommitParams(splitSign, stationArr[i].projectId,disposableProduct[j].productId, disposableProduct[j].productType, areaDiscountMarkTemp, productDiscountTemp,  allotUserArr[3], favourCode, rebatePercent, disposableMoney, stationArr[i].startTime, stationArr[i].endTime, 0, parentNo,seatArrIdStr,stationArr[i].timeEnough,orderNo,orderTime,allotUserArr[0],allotUserArr[1],disposableProduct[j].unit,disposableProduct[j].capacity,0,0,stationArr[i].type,disposableProduct[j].ratePriceFee,index++,null,stationArr[i].favorableLastMonth));
				}
			}
		}else{
			arr.push(new OrderCommitParams(splitSign, stationArr[i].projectId, stationArr[i].id,stationArr[i].productType, areaDiscountMarkTemp, productDiscountTemp,  allotUserArr[3], favourCode, rebatePercent, deposit, stationArr[i].startTime, stationArr[i].endTime,0, 0, seatArrIdStr,stationArr[i].timeEnough,orderNo,orderTime,allotUserArr[0],allotUserArr[1],stationArr[i].unitEnum,stationArr[i].volume,stationArr[i].margin,0,stationArr[i].type,stationArr[i].price,index++,null,stationArr[i].favorableLastMonth));
			if(stationArr[i].type==3){//disposableProduct
				for(var j=0;j<disposableProduct.length;j++){
					//disposableProductObj(productId,productName,validDate,expireDate,capacity,rateId,ratePriceFee,unit,productType){
					if(disposableProduct[j].projectId==stationArr[i].projectId&&disposableProduct[j].productType==10){
...
function findOrderInfoByContractId(contractId){
	...
	$.ajax({
		...
		success : function(data) {
			var stagesStr = "";
			if(data.payAction=="1") stagesStr = "月付";
			if(data.payAction=="2") stagesStr = "季付";
			if(data.payAction=="3") stagesStr = "半年付";
			if(data.payAction=="5") stagesStr = "全款";
			console.log(data);
			...
			if(data.contractInfo.companyName!=null){
				...
				$('.rent-contract').eq(1).find('div').eq(0).find('p').eq(1).html("<span style='box-sizing: border-box;width:140px;letter-spacing:0;font-weight: bold;font-size: 15px;margin-right: 10px;float: left;display: block;text-align: left;'>Client Name</span>"+data.contractInfo.companyName);
			}
			if(data.contractInfo.contractId!=null){
				...
				$('.rent-contract').eq(1).find('div').eq(0).find('p').eq(2).html("<span style='box-sizing: border-box;width:140px;letter-spacing:0;font-weight: bold;font-size: 15px;margin-right: 10px;float: left;display: block;text-align: left;'>Reference </span>NO."+data.contractInfo.contractId);
			}
			...
			if(data.contractInfo.finishtime!=null){
				...
				$('.rent-contract').eq(1).find('div').eq(0).find('p').eq(4).html("<span style='box-sizing: border-box;width:140px;letter-spacing:0;font-weight: bold;font-size: 15px;margin-right: 10px;float: left;display: block;text-align: left;'>Agreement date:</span>"+data.contractInfo.finishtime);
			}
			$('#contractHead').find('.rent-con').eq(0).find('p').eq(5).html("<span>付款周期</span>"+stagesStr);
			$('.rent-contract').eq(1).find('div').eq(0).find('p').eq(5).html("<span style='box-sizing: border-box;width:140px;letter-spacing:0;font-weight: bold;font-size: 15px;margin-right: 10px;float: left;display: block;text-align: left;'>Payment Plan	</span>"+stagesStr);
			...
			if(data.loginUser!=null){
				if(data.sellerInfo!=null){
					if(data.sellerInfo.adminLname!=null){
						$('#contractHead').find('.rent-con').eq(1).find('p').eq(0).html("<span style='width: 60px;font-weight: bold;font-size: 15px;margin-right: 10px;'>联系人</span>"+data.sellerInfo.adminRname);
						$('.rent-contract').eq(1).find('div').eq(1).find('p').eq(0).html("<span style='box-sizing: border-box;width:110px;letter-spacing:0;;font-weight: bold;font-size: 15px;margin-right: 10px;'>Contact Name</span>"+data.sellerInfo.adminRname);
					}
					if(data.sellerInfo.adminPhone!=null){
						$('#contractHead').find('.rent-con').eq(1).find('p').eq(1).html("<span style='width: 60px;font-weight: bold;font-size: 15px;margin-right: 10px;'>电话</span>"+data.sellerInfo.adminPhone);
						$('.rent-contract').eq(1).find('div').eq(1).find('p').eq(1).html("<span style='box-sizing: border-box;width:110px;letter-spacing:0;;font-weight: bold;font-size: 15px;margin-right: 10px;'>Tel</span>"+data.sellerInfo.adminPhone);
					}
						$('#contractHead').find('.rent-con').eq(1).find('p').eq(2).html("<span style='width: 60px;font-weight: bold;font-size: 15px;margin-right: 10px;'>职位</span>"+"销售");
						$('.rent-contract').eq(1).find('div').eq(1).find('p').eq(2).html("<span style='box-sizing: border-box;width:110px;letter-spacing:0;;font-weight: bold;font-size: 15px;margin-right: 10px;'>Position</span>"+"Salesman");
					if(data.sellerInfo.adminEmail!=null){
						$('#contractHead').find('.rent-con').eq(1).find('p').eq(3).html("<span style='width: 60px;font-weight: bold;font-size: 15px;margin-right: 10px;'>邮箱</span>"+data.sellerInfo.adminEmail);
						$('.rent-contract').eq(1).find('div').eq(1).find('p').eq(3).html("<span style='box-sizing: border-box;width:110px;letter-spacing:0;;font-weight: bold;font-size: 15px;margin-right: 10px;'>E-mail</span>"+data.sellerInfo.adminEmail);
					}
						$('#contractHead').find('.rent-con').eq(1).find('p').eq(4).html("<span style='width: 70px;font-weight: bold;font-size: 15px;margin-right: 10px;'>账单地址</span>");
						$('.rent-contract').eq(1).find('div').eq(1).find('p').eq(4).html("<span style='box-sizing: border-box;width:140px;lette-spacing:0;font-weight: bold;font-size: 15px;margin-right: 10px;'>Mailing Address</span>");
				}else{
					if(data.loginUser.adminLname!=null){
						$('#contractHead').find('.rent-con').eq(1).find('p').eq(0).html("<span style='width: 60px;font-weight: bold;font-size: 15px;margin-right: 10px;'>联系人</span>"+data.loginUser.adminRname);
						$('.rent-contract').eq(1).find('div').eq(1).find('p').eq(0).html("<span style='box-sizing: border-box;width:110px;letter-spacing:0;;font-weight: bold;font-size: 15px;margin-right: 10px;'>Contact Name</span>"+data.loginUser.adminRname);						
					}
					if(data.loginUser.adminPhone!=null){
						$('#contractHead').find('.rent-con').eq(1).find('p').eq(1).html("<span style='width: 60px;font-weight: bold;font-size: 15px;margin-right: 10px;'>电话</span>"+data.loginUser.adminPhone);
						$('.rent-contract').eq(1).find('div').eq(1).find('p').eq(1).html("<span style='box-sizing: border-box;width:110px;letter-spacing:0;;font-weight: bold;font-size: 15px;margin-right: 10px;'>Tel</span>"+data.loginUser.adminPhone);						
					}
						$('#contractHead').find('.rent-con').eq(1).find('p').eq(2).html("<span style='width: 60px;font-weight: bold;font-size: 15px;margin-right: 10px;'>职位</span>"+"销售");
						$('.rent-contract').eq(1).find('div').eq(1).find('p').eq(2).html("<span style='box-sizing: border-box;width:110px;letter-spacing:0;;font-weight: bold;font-size: 15px;margin-right: 10px;'>Position</span>"+"Salesman");
					if(data.loginUser.adminEmail!=null){
						$('#contractHead').find('.rent-con').eq(1).find('p').eq(3).html("<span style='width: 60px;font-weight: bold;font-size: 15px;margin-right: 10px;'>邮箱</span>"+data.loginUser.adminEmail);
						$('.rent-contract').eq(1).find('div').eq(1).find('p').eq(3).html("<span style='box-sizing: border-box;width:110px;letter-spacing:0;;font-weight: bold;font-size: 15px;margin-right: 10px;'>E-mail</span>"+data.loginUser.adminEmail);
					}
						$('#contractHead').find('.rent-con').eq(1).find('p').eq(4).html("<span style='width: 70px;font-weight: bold;font-size: 15px;margin-right: 10px;'>账单地址</span>");
						$('.rent-contract').eq(1).find('div').eq(1).find('p').eq(4).html("<span style='box-sizing: border-box;width:110px;letter-spacing:0;;font-weight: bold;font-size: 15px;margin-right: 10px;'>Mailing Address</span>");
				}
			}
			
			if(data.contractInfo!=null){
				if(data.contractInfo.userName!=null){
					$('#contractHead').find('.rent-con').eq(2).find('p').eq(0).html("<span style='width: 60px;font-weight: bold;font-size: 15px;margin-right: 10px;'>联系人</span>"+data.contractInfo.userName);
					$('.rent-contract').eq(1).find('div').eq(2).find('p').eq(0).html("<span style='box-sizing: border-box;width:110px;letter-spacing:0;;font-weight: bold;font-size: 15px;margin-right: 10px;'>Contact Name</span>"+data.contractInfo.userName);
				}
				if(data.contractInfo.userPhone!=null){
					$('#contractHead').find('.rent-con').eq(2).find('p').eq(1).html("<span style='width: 60px;font-weight: bold;font-size: 15px;margin-right: 10px;'>电话</span>"+data.contractInfo.userPhone);
					$('.rent-contract').eq(1).find('div').eq(2).find('p').eq(1).html("<span style='box-sizing: border-box;width:110px;letter-spacing:0;;font-weight: bold;font-size: 15px;margin-right: 10px;'>Tel</span>"+data.contractInfo.userPhone);
				}
				$('#contractHead').find('.rent-con').eq(2).find('p').eq(2).html("<span style='width: 60px;font-weight: bold;font-size: 15px;margin-right: 10px;'>职位</span>");
				$('.rent-contract').eq(1).find('div').eq(2).find('p').eq(2).html("<span style='box-sizing: border-box;width:110px;letter-spacing:0;;font-weight: bold;font-size: 15px;margin-right: 10px;'>Position</span>"+"Salesman");
				if(data.contractInfo.userMail!=null){
					$('#contractHead').find('.rent-con').eq(2).find('p').eq(3).html("<span style='width: 60px;font-weight: bold;font-size: 15px;margin-right: 10px;'>邮箱</span>"+data.contractInfo.userMail);
					$('.rent-contract').eq(1).find('div').eq(2).find('p').eq(3).html("<span style='box-sizing: border-box;width:110px;letter-spacing:0;;font-weight: bold;font-size: 15px;margin-right: 10px;'>E-mail</span>"+data.contractInfo.userMail);
				}
				if(data.contractInfo.companyAddress!=null){
					$('#contractHead').find('.rent-con').eq(2).find('p').eq(4).html("<span style='width: 60px;font-weight: bold;font-size: 15px;margin-right: 10px;'>账单地址</span>"+data.contractInfo.companyAddress);
					$('.rent-contract').eq(1).find('div').eq(2).find('p').eq(4).html("<span style='box-sizing: border-box;width:110px;letter-spacing:0;;font-weight: bold;font-size: 15px;margin-right: 10px;'>Mailing Address</span>"+data.contractInfo.companyAddress);					
				}
			}
			...
			for(var i=0;i<list.length;i++){
				for(var j=0;j<list[i].length;j++){
					...
					if(productType==1||productType==3){
						...
						/*price = list[i][j].ensureFee/list[i][j].amount;
						rent = list[i][j].ensureFee * list[i][j].capacity;*/
					...
					if(j==0){
						...
						htm += list[i][j].projectName+"</td><td style='line-height: 30px;vertical-align: middle;'>"+list[i][j].productName+"("+list[i][j].capacity+")</br>"+assetNumStr+"</td><td style='line-height: 30px;vertical-align: middle;'>"+moneyMark+""+price+""+unitStrSon+"</td><td style='line-height: 30px;vertical-align: middle;'>"+list[i][j].amount+"</td><td>"+list[i][j].rentStartTime+"到"+list[i][j].rentEndTime+"</td><td style='line-height: 30px;vertical-align: middle;'>"+moneyMark+""+rent+""+unitStr+"</td></tr>";
					}else{
						htm+="<tr><td valign='top' style='line-height: 30px;vertical-align: middle;'>";
						htm += list[i][j].productName+"("+list[i][j].capacity+")</br>"+assetNumStr+"</td><td style='line-height: 30px;vertical-align: middle;'>"+moneyMark+""+price+""+unitStrSon+"</td><td style='line-height: 30px;vertical-align: middle;'>"+list[i][j].amount+"</td><td style='line-height: 30px;vertical-align: middle;'>"+list[i][j].rentStartTime+"到"+list[i][j].rentEndTime+"</td><td style='line-height: 30px;vertical-align: middle;'>"+moneyMark+""+rent+""+unitStr+"</td></tr>";
					}
					
					if(list[i][j].productType==2) meetingRoomPackage= meetingRoomPackage+list[i][j].rent;
					if(list[i][j].productType==4) elasticStationJuan= elasticStationJuan+list[i][j].rent;
					if(list[i][j].parentId==0){
						var splitList = list[i][j].splitList;
						//firstMoney += splitList[0].priceFee-list[i][j].ensureFee;
					...
					if(list[i][j].productType==6||list[i][j].productType==7){
						/*var cut = list[i][j].zhihuibaoMoney;
						jianmian +=  Math.ceil(cut);*/
						var splitList = list[i][j].splitList;
						if(splitList.length>0)
			...
			/*if(list[0][0].crossProjectDiscount!=1&&list[0][0].crossProjectDiscount!=null){//跨点组合优惠
				...
				htm+="</td><td style='line-height: 30px;vertical-align: middle;'>跨点折扣</td><td style='line-height: 30px;vertical-align: middle;'></td><td style='line-height: 30px;vertical-align: middle;'></td><td style='line-height: 30px;vertical-align: middle;'></td><td style='line-height: 30px;vertical-align: middle;'>"+list[0][0].crossProjectDiscount+"% off</td></tr>"
			}
			if(list[0][0].crossProductDiscount!=1&&list[0][0].crossProductDiscount!=null){//跨产品组合优惠
				...
				htm+="</td><td style='line-height: 30px;vertical-align: middle;'>跨产品折扣</td><td style='line-height: 30px;vertical-align: middle;'></td><td style='line-height: 30px;vertical-align: middle;'></td><td style='line-height: 30px;vertical-align: middle;'></td><td style='line-height: 30px;vertical-align: middle;'>"+list[0][0].crossProductDiscount+"% off</td></tr>";
			}*/
			if(list[0][0].payActionDiscount!=1&&list[0][0].payActionDiscount!=null){//跨产品组合优惠
				...
				htm+="</td><td style='line-height: 30px;vertical-align: middle;'>付款节奏折扣</td><td style='line-height: 30px;vertical-align: middle;'></td><td style='line-height: 30px;vertical-align: middle;'></td><td style='line-height: 30px;vertical-align: middle;'></td><td style='line-height: 30px;vertical-align: middle;'>"+list[0][0].payActionDiscount+"% off</td></tr>";
			}
			if(list[0][0].favourDiscount!=1&&list[0][0].favourDiscount!=null){//优惠码
				...
				htm+="</td><td style='line-height: 30px;vertical-align: middle;'>优惠码折扣</td><td style='line-height: 30px;vertical-align: middle;'></td><td style='line-height: 30px;vertical-align: middle;'></td><td style='line-height: 30px;vertical-align: middle;'></td><td style='line-height: 30px;vertical-align: middle;'>"+list[0][0].favourDiscount+"% off</td></tr>";
			}
			if(firstMoney!=0){
			...
			var totalStr = "";
			if(total!=0) totalStr = totalStr + total +".00元";
			if(meetingRoomPackage!=0) totalStr = totalStr +"  " + meetingRoomPackage+"张会议室券";
			if(elasticStationJuan!=0) totalStr = totalStr +"  " + elasticStationJuan+"张弹性工位劵";
...
function getFirstMoney(){
	...
	var arr = [];
	arr.push(new OrderCommitParams(splitSign, stationArr[i].projectId,stationArr[i].discountProductId, stationArr[i].discountProductType, areaDiscountMarkTemp, productDiscountTemp,  allotUserArr[3], favourCode, rebatePercent, discountPrice, stationArr[i].startTime, stationArr[i].endTime, 0, parentNo,seatArrIdStr,stationArr[i].timeEnough,orderNo,orderTime,allotUserArr[0],allotUserArr[1],stationArr[i].packageUnit,stationArr[i].volume,0,0,stationArr[i].packageType,stationArr[i].packageDiscountPrice,index++,"no"));
	$.ajax({
		url : 'filter/offlineReserve/saveOrderInfo',
		type : 'post',
		data : {
...
function pushStationArrData(obj){
	if(stationArr.length==0){
		...
		if(obj.existPackage=="yes"){
			...
			for(var i=0;i<stationArr.length;i++){
				if(stationArr[i].id==obj.id&&stationArr[i].type==obj.type&&stationArr[i].startTime==obj.startTime&&stationArr[i].endTime==obj.endTime){
...
function packageUpInfoTest(){
	...
	$.ajax({
		...
		success : function(data) {
			var htm = "";
			htm+="<div class='server grey-server upPackage' id='upPackage"+packageUp[0].productId+"' currentProductId='"+packageUp[0].productId+"' >"+packageUp[0].projectName+"--"+packageUp[0].productName+"智慧包升级<select class='form-control input-md' onchange='clickPackageUpOption(this)'>";
			for(var x=0;x<data.length;x++){
				htm += "<option productId='"+data[x].productId+"' productType='"+data[x].productType+"' money='"+data[x].money+"' unitEnum='"+data[x].unitEnum+"'>"+data[x].productName+"</option>";
			}
			htm += "</select></div>";
...
function clickPackageUpOption(currentThis){
	...
	var $upPackageObj = new upPackageObj(packageId,packageName,packageType,packagePrice,packageUnit,discountProductId,packageDiscountPrice,discountProductType);
	...
	//new upPackageDiscountObj(upPackageId,upPackageName,upPackageType,upPackageDiscountPrice,upPackageUnit)
	console.log($upPackageObj)
	/*if(data.length>0)
	var $newObj = new upPackageDiscountObj(data[0].productId,data[0].productName,data[0].productType,data[0].money,data[0].unit);*/
	for(var i=0;i<stationArr.length;i++){
		if(stationArr[i].index == objStoryIndex){
			stationArr[i].favorableLastMonth = false;
...
function createPackageUpInfo(packageIsUpArr){
	...
	//findUpPackage.remove();
	for(var i=0;i<packageIsUpArr.length;i++){
		var mark = true;
		var tag = true;
		for(var j=0;j<findUpPackage.length&&tag;j++){
			...
			if(index == packageIsUpArr[i].index){
				for(var x=0;x<stationArr.length;x++){
					if(stationArr[x].index==index){
						var il = findUpPackage.eq(j).attr('incrementlevel');
						if(stationArr[x].packageIncrementLevel==il){
							//没有重新选择智慧包
							mark = false;
							tag = false;
							break;
							...
							if(stationArr[i].upPackageObj!=null){
								stationArr[x].upPackageObj == null;
								stationArr[x].upPackageDiscountObj = null;
							}
							break;
						}
					}
		...
		if(mark){
			...
			for(var j=0;j<IncrementArr.length;j++){
				if(IncrementArr[j].productId==packageIsUpArr[i].packageId){
					...
					if(IncrementArr[j].incrementLevel<3){
						flag= true;
					}else if(IncrementArr[j].incrementLevel==3){
						for(var x=0;x<stationArr.length;x++){
							if(stationArr[x].index==packageIsUpArr[i].index){
								stationArr[x].favorableLastMonth = true;
							}
						}
					}
				}
			...
			if(flag){
				htm+="<div class='server grey-server upPackage' objStoryIndex='"+packageIsUpArr[i].index+"' incrementLevel='"+incrementLevel+"' packageId='"+packageIsUpArr[i].packageId+"' currentProductId='"+packageIsUpArr[i].productId+"' >"+packageIsUpArr[i].projectName+"  "+packageIsUpArr[i].productName+"  "+packageIsUpArr[i].packageName+"<select class='form-control input-md' onchange='clickPackageUpOption(this)' >";
				...
				for(var j=0;j<IncrementArr.length;j++){
					...
					console.log(parseInt(incrementLevel)+2)
					//htm+="<input type='hidden' class='inputVal' incrementLevel  productId='"+data[i].incrementProductOutsList[j].productId+"' ratePriceFee='"+data[i].incrementProductOutsList[j].ratePriceFee+"' unit='"+data[i].incrementProductOutsList[j].unit+"' discountPrice='"+data[i].incrementProductOutsList[j].discountPrice+"' discountProductType='"+data[i].incrementProductOutsList[j].discountProductType+"' discountProductId='"+data[i].incrementProductOutsList[j].discountProductId+"' incrementProductName='"+data[i].incrementProductOutsList[j].incrementProductName+"' >"	
					if(IncrementArr[j].incrementLevel>=incrementLevel&&IncrementArr[j].incrementLevel<(parseInt(incrementLevel)+2)){
						if(IncrementArr[j].incrementLevel==incrementLevel){
							htm += "<option favorableMark='false' default='true' objStoryIndex='"+packageIsUpArr[i].index+"' incrementLevel='"+incrementLevel+"' projectId='"+packageIsUpArr[i].projectId+"' productId='"+productId+"' packageProductId='"+IncrementArr[j].productId+"' ratePriceFee='"+IncrementArr[j].ratePriceFee+"' unit='"+IncrementArr[j].unit+"' discountPrice='"+IncrementArr[j].discountPrice+"' discountProductType='"+IncrementArr[j].discountProductType+"' discountProductId='"+IncrementArr[j].discountProductId+"'  >保持产品及价格不变</option>";
						}else{
							//htm += "<option favorableMark='false default='false' objStoryIndex='"+packageIsUpArr[i].index+"' incrementLevel='"+incrementLevel+"' projectId='"+packageIsUpArr[i].projectId+"' productId='"+productId+"' packageProductId='"+IncrementArr[j].productId+"' ratePriceFee='"+IncrementArr[j].ratePriceFee+"' unit='"+IncrementArr[j].unit+"' discountPrice='"+IncrementArr[j].discountPrice+"' discountProductType='"+IncrementArr[j].discountProductType+"' discountProductId='"+IncrementArr[j].discountProductId+"'  >"+IncrementArr[j].incrementProductName+"</option>";
							htm += "<option favorableMark='false default='false' objStoryIndex='"+packageIsUpArr[i].index+"' incrementLevel='"+incrementLevel+"' projectId='"+packageIsUpArr[i].projectId+"' productId='"+productId+"' packageProductId='"+IncrementArr[j].productId+"' ratePriceFee='"+IncrementArr[j].ratePriceFee+"' unit='"+IncrementArr[j].unit+"' discountPrice='"+IncrementArr[j].discountPrice+"' discountProductType='"+IncrementArr[j].discountProductType+"' discountProductId='"+IncrementArr[j].discountProductId+"'  >免费升级IT智慧包</option>";
						}
					}
				}
	...
	for(var i=0;i<stationArr.length;i++){
		...
		if(mark&&stationArr[i].seatArr.length>0){
			...
			for(var k=0;k<stationArr[i].seatArr.length;k++){
				...
				if((k+1)%4==0)seatArrStr+="</br>"
			}
			var calculateResult = stationArr[i].calculateResult;
			var calculateResultStr = "";
			if(stationArr[i].productType==2) calculateResultStr = (Math.floor(calculateResult)) +"个会议室券";
			if(stationArr[i].productType==4) calculateResultStr = (Math.floor(calculateResult)) +"张弹性工位劵";
			if(stationArr[i].productType==1||stationArr[i].productType==3) calculateResultStr = "￥"+(Math.floor(calculateResult));
			var favourPrice = getDiscountPrice(stationArr[i].price, favourCode, stationArr[i].productType, stationArr[i].id);
			//htm += "<tr class='orderDetail' productId='"+stationArr[i].id+"'><td>"+stationArr[i].projectName+"</td><td>"+stationArr[i].productName+"</td><td>"+spaceType+"</td><td>"+stationArr[i].seatArr+"</td><td>￥"+stationArr[i].price+"/"+stationArr[i].unit+"</td><td>"+stationArr[i].startTime+"到"+stationArr[i].endTime+"</td><td>"+calculateResultStr+"</td></tr>";
			htm += "<tr class='orderDetail' productId='"+stationArr[i].id+"'><td>"+stationArr[i].projectName+"</td><td>"+stationArr[i].productName+"</td><td>"+spaceType+"</td><td>"+seatArrStr+"</td><td>￥"+favourPrice+"/"+stationArr[i].unit+"</td><td>"+stationArr[i].startTime+"到"+stationArr[i].endTime+"</td><td>"+calculateResultStr+"</td></tr>";
			if(stationArr[i].packageName!=null){
				if(stationArr[i].upPackageObj!=null){
					var favourPackagePrice = getDiscountPrice(stationArr[i].upPackageObj.packagePrice, favourCode, stationArr[i].upPackageObj.packageType, stationArr[i].upPackageObj.packageId);
					htm += "<tr class='z-tr'><td colspan='6'><div class='z-tr-d'><div class='z-round'></div><div class='z-line'></div>" +
					"<p class='z-tr-p'><span>--"+stationArr[i].upPackageObj.packageName+"</span><span>"+favourPackagePrice+"/"+stationArr[i].unit+"</span><span>服务费：￥"+Math.floor(stationArr[i].calculatePackageResult)+"</span>"
				}else{
					var favourPackagePrice = getDiscountPrice(stationArr[i].packagePrice, favourCode, stationArr[i].packageType, stationArr[i].packageId);
					htm += "<tr class='z-tr'><td colspan='6'><div class='z-tr-d'><div class='z-round'></div><div class='z-line'></div>" +
					"<p class='z-tr-p'><span>--"+stationArr[i].packageName+"</span><span>"+favourPackagePrice+"/"+stationArr[i].unit+"/人"+"</span><span>服务费：￥"+Math.floor(stationArr[i].calculatePackageResult)+"</span>"
				}
			...
			if(stationArr[i].margin!=0||stationArr[i].disposableCost!=0){
				htm += "<tr class='z-tr'><td colspan='6'><div class='z-tr-d'><div class='z-round'></div><div class='z-line'></div>" +
				"<p class='z-tr-p'><span>--保证金</span><span>￥<input type='text' class='marginInput' onMouseOut='modifyMargin()' value="+(stationArr[i].margin)+" ></span>"
				htm+="</p></div></td></tr>";
				if(stationArr[i].disposableCost!=0){
					htm += "<tr class='z-tr'><td colspan='6'><div class='z-tr-d'><div class='z-round'></div><div class='z-line'></div>";
					htm+="<p class='z-tr-p'><span>--一次性费用</span><span>"+stationArr[i].disposableCostPrice+"/"+stationArr[i].unit+"/m²</span><span>￥"+stationArr[i].disposableCost+"</span>";
					htm+="</p>";
					htm+="</div></td></tr>";
				}
...
function judgePackageIsUp(){
	...
	for(var i=0;i<stationArr.length;i++){
		if(stationArr[i].timeEnough=="yes"&&stationArr[i].existPackage=="yes"&&stationArr[i].volume*stationArr[i].seatArr.length>=1){
			packageUp.push(new ObjPackageUp(stationArr[i].index,stationArr[i].projectId,stationArr[i].projectName,stationArr[i].id,stationArr[i].productName,stationArr[i].packageId,stationArr[i].packageName,stationArr[i].endTime))
		}
	}
	//选择一人次条件
	return packageUp;
	if(stationArr.length==1){
		if(stationArr[0].timeEnough=="yes"&&stationArr[0].existPackage=="yes"&&stationArr[0].volume*stationArr[0].seatArr.length>=1){
			packageUp.push(new ObjPackageUp(stationArr[0].index,stationArr[0].projectId,stationArr[0].projectName,stationArr[0].id,stationArr[0].productName,stationArr[0].packageId,stationArr[0].packageName,stationArr[0].endTime))
		}
		return packageUp;
	}
	...
	for(var i=0;i<stationArr.length;i++){
		if(stationArr[i].timeEnough=="yes"&&stationArr[i].existPackage=="yes"){
			...
			for(var j=0;j<stationArr.length;j++){
				var mark = true;
				if(j!=i&&stationArr[j].timeEnough=="yes"&&stationArr[j].existPackage=="yes"&&stationArr[i].startTime==stationArr[j].startTime&&stationArr[i].endTime==stationArr[j].endTime){
					if(mark){
				...
				if(j!=i&&stationArr[j].timeEnough=="yes"&&stationArr[j].existPackage=="yes"&&stationArr[i].startTime==stationArr[j].startTime&&stationArr[i].endTime==stationArr[j].endTime){
					if(num>=10){
						if(packageUp.length==0){
							packageUp.push(new ObjPackageUp(stationArr[i].index,stationArr[i].projectId,stationArr[i].projectName,stationArr[i].id,stationArr[i].productName,stationArr[i].packageId,stationArr[i].packageName,stationArr[i].endTime))
							packageUp.push(new ObjPackageUp(stationArr[j].index,stationArr[j].projectId,stationArr[j].projectName,stationArr[j].id,stationArr[j].productName,stationArr[j].packageId,stationArr[j].packageName,stationArr[j].endTime))
						}else{
							for(var x=0;x<packageUp.length;x++){
								if(stationArr[i].id==packageUp[x].productId) break;
							...
							for(var x=0;x<packageUp.length;x++){
								if(stationArr[j].id==packageUp[x].productId) break;
								if(stationArr[j].id!=packageUp[x].productId&&x==packageUp.length-1){
									packageUp.push(new ObjPackageUp(stationArr[j].index,stationArr[j].projectId,stationArr[j].projectName,stationArr[j].id,stationArr[j].productName,stationArr[j].packageId,stationArr[j].packageName,stationArr[i].endTime))
									break;
								}
							}
						}
...
function orderNoAndTime(){
 ...
 function ObjStory(index,id, projectName,projectId,productName,biaoti, type,seatArr, seatArrId,price,volume,packageType,packageName,packagePrice,packageId,packageIncrementLevel,discountProductId,timeEnough,startTime,endTime,unit,unitEnum,packageUnit,existPackage,calculateResult,packageFavor,calculatePackageResult,margin,packageDiscountPrice,packageNoId,productType,productArea,disposableCost,discountProductType,upPackageObj,upPackageDiscountObj,telecomSetCost,favorableLastMonth,disposableCostPrice,oneMonthMoney) // 声明对象
{
	this.index = index;
	this.id = id;
	this.projectName = projectName;
		...
		for(var j=0;j<stationArr.length;j++){
			if(page2Product.length==0){
				if(stationArr[j].projectName == projectArr[i]&&stationArr[j].seatArr.length>0){
					htm+="<div class='page2ProductShow' productId='"+stationArr[j].id+"' class='z-tr-d'><div class='z-round z-p-r'></div><div class='z-line z-p-l'></div>"+
					"<div class='tl'><b>"+stationArr[j].productName+"("+stationArr[j].biaoti+")</b><p>" ;
				...
				for(var h=0;h<page2Product.length;h++){
					if(page2Product.eq(h).attr('productId')!=stationArr[j].id){
						if(stationArr[j].projectName == projectArr[i]&&stationArr[j].seatArr.length>0){
							htm+="<div class='page2ProductShow' productId='"+stationArr[j].id+"' class='z-tr-d'><div class='z-round z-p-r'></div><div class='z-line z-p-l'></div>"+
							"<div class='tl'><b>"+stationArr[j].biaoti+"</b><p>" ;
							for(var x=0;x<stationArr[j].seatArr.length;x++){
								if(x!=stationArr[j].seatArr.length-1){
									htm+=stationArr[j].seatArr[x]+",";
								}else{
...
function clickUpDown(){
	...
	$(".company-date li:not(:last)").click(function(){
		...
		if($(this).html().trim()!="+新增公司"){
			$(this).parent().parent().parent().find(".filling-name").html(companyNameStr.substring(3,companyNameStr.length-4)+"<span class='caret'></span>");
		}
			...
			for(var i=0;i<5;i++){
				for(var j=0;j<companyDateLi.length;j++){
					...
					if(companyId==data[i].companyId){
						htm+= "<tr class='person-list companyTr' companyId='"+data[i].companyId+"' ><td><i class='glyphicon glyphicon-user'></i></td><td class='td-name'>"+data[i].companyName+"</td>"+
...
function getCompanyInfoByPhone(t,userPhone){
	$.ajax({
		...
		success : function(data) {
			...
			for(var j=0;j<companyDateLi.length;j++){
				var companyId = companyDateLi.eq(j).attr('companyId');
				if(companyId==data[0].companyId){
					htm+= "<tr class='person-list companyTr' companyId='"+data[0].companyId+"' ><td><i class='glyphicon glyphicon-user'></i></td><td class='td-name'>"+data[0].companyName+"</td>"+
					"<td class='td-number'>AD1120134</td><td><button type='button' class='btn btn-default bts' onclick='sureSelectCompany(this)'>已选择</button></td></tr>"
					break;
				}
				if(j==companyDateLi.length-1){
